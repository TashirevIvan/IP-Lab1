...
Mat gray_Conv(Mat& res_Mat) {
	...
	for (int i = 0; i < res_Mat.rows; i++) {
		for (int j = 0; j < res_Mat.cols; j++) {
			res_Mat.at<Vec3b>(i, j)[0] = (res_Mat.at<Vec3b>(i, j)[0] + res_Mat.at<Vec3b>(i, j)[1] + res_Mat.at<Vec3b>(i, j)[2]) / 3;
			res_Mat.at<Vec3b>(i, j)[1] = (res_Mat.at<Vec3b>(i, j)[0] + res_Mat.at<Vec3b>(i, j)[1] + res_Mat.at<Vec3b>(i, j)[2]) / 3;
			res_Mat.at<Vec3b>(i, j)[2] = (res_Mat.at<Vec3b>(i, j)[0] + res_Mat.at<Vec3b>(i, j)[1] + res_Mat.at<Vec3b>(i, j)[2]) / 3;
		}
	}
	return res_Mat;
}
